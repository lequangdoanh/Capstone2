...
export const BlogCard = ({ blog, user, setSignInOpen }) => {
  const [favourite, setFavourite] = useState(false)
  const dispatch = useDispatch();

  const token = localStorage.getItem("podstreamtoken");

  const favoritblog = async () => {
    await favoritePodcast(blog._id, token).then((res) => {
      if (res.status === 200) {
  ...
  React.useEffect(() => {
    ...
    if (user?.favorits?.find((fav) => fav._id === blog._id)) {
      setFavourite(true)
    }
  }, [user])
    ...
    <Card to={`/blog/${blog._id}`}>
      <div>
        ...
        <Top>
          ...
          <CardImage src={blog.thumbnail} />
        </Top>
        <CardInformation>
          <MainInfo>
            <Title>{blog.name.slice(0,21)}</Title>
            <Description>{blog.desc}</Description>
            <CreatorInfo>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <Avatar
                  src={blog?.creator?.img} style={{ width: '26px', height: '26px' }}>{blog?.creator?.name?.charAt(0).toUpperCase()}</Avatar>
                <CreatorName>
                  {blog?.creator?.name}
                </CreatorName>

              </div>
              <Views>• {blog.views} Views</Views>
            </CreatorInfo>
          </MainInfo>
        </CardInformation>
      </div>
